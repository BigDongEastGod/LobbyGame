...
namespace ETHotfix
{
    ...
    public class NiuNiuMainComponent : Component
    {
        ...
        private async void SetRoomInfo(long roomId, bool isSitDown)
        {
            ...
            //亮牌按钮
            var brightButton = rc.Get<GameObject>("BrightCardsButton");
            ...
            //获取当前账号
            var accountResponse =
                (GetAccountInfoResponse) await SceneHelperComponent.Instance.Session.Call(new GetAccountInfoRequest());
            _player = accountResponse;

            //请求获得当前房间准备好的玩家信息
            var response =
                (RoomInfoResponse) await SceneHelperComponent.Instance.Session.Call(
                    new RoomInfoRequest() {RoomId = roomId});
            _roomInfo = response;
            //获得房间规则信息
            ...
            //开始按钮注册
            SceneHelperComponent.Instance.MonoEvent.AddButtonClick(_startGameBt.GetComponent<Button>(),
                StartGameOclick);
