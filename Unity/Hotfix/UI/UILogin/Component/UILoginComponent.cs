...
//			this.account = rc.Get<GameObject>("Account");
			var button1 = rc.Get<GameObject>("Button");
			SceneHelperComponent.Instance.MonoEvent.AddButtonClick(button1.GetComponent<Button>(),OnButton1Click);
		}
		...
		private async void OnButton1Click()
		{
			...
			var response = (LoginResponse) await session.Call(new LoginRequest() {UserName = "user", Password ="123"});
			
			session.Dispose();

			session = await SceneHelperComponent.Instance.CreateGateSession(response.Address, response.Key);

			var gateresponse = (GetAccountInfoResponse) await session.Call(new GetAccountInfoRequest());
			
			Debug.Log(gateresponse.Error);
			
			Debug.Log(gateresponse.AccountInfo);

//			if (response.Error == 0)
//			{
//				Debug.Log(response.Address);
//			}
//			else
//			{
