...
namespace ETModel
{
    ...
    public class UIManageComponent : Component
    {
	    ...
	    public override void Dispose()
	    {
		    ...
		    foreach (var type in uis.Keys.ToArray())
		    {
			    UI ui;
				
			    if (!uis.TryGetValue(type, out ui)) continue;
		...
		{
			foreach (var type in this.uis.Keys.ToArray())
			{
				UI ui;
				
				if (!this.uis.TryGetValue(type, out ui)) continue;
