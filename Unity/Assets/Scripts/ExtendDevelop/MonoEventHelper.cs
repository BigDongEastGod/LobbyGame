...
namespace ETModel
 {
     public class MonoEventHelper
     {
         private static MonoEventHelper _instance;

         public static MonoEventHelper Instance => _instance ?? (_instance = new MonoEventHelper());

         public MonoEventHelper()
         {
         ...
         public EventTrigger AddEventTrigger( GameObject obj, EventTriggerType eventTriggerType, bool isRemoveAllListeners, bool isClearTriggers, Action<BaseEventData> action)
         {
             ...
             var callback = new UnityAction<BaseEventData>(action); entry.callback.AddListener(callback); if (isClearTriggers) eventTrigger.triggers.Clear();
